...
export const BookingsManager: React.FC<BookingsManagerProps> = ({ bookings, loading, onRefresh }) => {
    ...
    const filteredBookings = useMemo(() => {
        let filtered = bookings.filter(b => b.estado !== BOOKING_STATUS.CANCELADO);
    ...
    const handleDrop = async (e: React.DragEvent, targetDate: Date, targetHour: number) => {
        ...
        const isSlotTaken = bookings.some(b => {
            ...
            if (b.estado === BOOKING_STATUS.CANCELADO) return false;
...
const DraggableBookingCard: React.FC<{
    ...
    return (
        ...
        ${booking.estado === BOOKING_STATUS.CONFIRMADO
                    ? 'bg-amber-500 text-black border-amber-600 hover:bg-amber-400'
                    : booking.estado === BOOKING_STATUS.COMPLETADO
