...
export const DashboardView: React.FC<DashboardViewProps> = ({ bookings, loading }) => {
    ...
    const loadBranches = () => {
        bookingService.getBranches().then(setBranches);
    };
    ...
    const handleStatusChange = async (branchId: string, status: BranchStatus) => {
        ...
        await bookingService.updateBranchStatus(branchId, status);
    ...
    const stats = useMemo(() => {
        const activeBookings = filteredBookings.filter(b => b.estado !== BOOKING_STATUS.CANCELADO);

        const totalRevenue = activeBookings.reduce((sum, b) => sum + b.servicio.precio, 0);
        const confirmedCount = filteredBookings.filter(b => b.estado === BOOKING_STATUS.CONFIRMADO).length;
