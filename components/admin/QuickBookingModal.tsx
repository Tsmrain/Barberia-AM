...
export const QuickBookingModal: React.FC<QuickBookingModalProps> = ({ isOpen, onClose, onSuccess }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            Promise.all([bookingService.getBranches(), bookingService.getServices()])
                .then(([b, s]) => {
                    setBranches(b);
                    setServices(s);
                    // Default selections
    ...
    useEffect(() => {
        if (selectedBranchId) {
            bookingService.getBarbers(selectedBranchId).then(data => {
                setBarbers(data);
                if (data.length > 0) {
    ...
    useEffect(() => {
        ...
        const checkPhone = async () => {
            if (isValidLength) {
                setIsChecking(true);
                try {
                    const found = await bookingService.checkClientByPhone(phone);
                    setClient(found);
                } finally {
                    setIsChecking(false);
                }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const takenSlots = await bookingService.getTakenSlots(selectedDateTime, selectedBarberId);
            ...
            if (!currentClient) {
                ...
                currentClient = await bookingService.createClient(phone, name);
            }
