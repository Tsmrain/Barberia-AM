...
export const EditBookingModal: React.FC<EditBookingModalProps> = ({
    ...
    const [status, setStatus] = useState<BookingStatus>(BOOKING_STATUS.PENDIENTE);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await bookingService.updateBookingDetails(booking.id, {
                date: fullDate,
                time: selectedTime,
                serviceId,
                barberId,
                ...
                <div className="p-6 border-b border-white/5 flex justify-between items-center bg-[#0a0a0a] shrink-0">
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            ${booking.estado === BOOKING_STATUS.CONFIRMADO ? 'bg-green-500/20 text-green-500' :
                                    booking.estado === BOOKING_STATUS.COMPLETADO ? 'bg-blue-500/20 text-blue-500' :
                                        booking.estado === BOOKING_STATUS.NO_SHOW ? 'bg-red-500/20 text-red-500' : 'bg-amber-500/20 text-amber-500'}
                        `}>
                                {(booking.estado as string).replace('_', ' ')}
                            </span>
                        </div>
