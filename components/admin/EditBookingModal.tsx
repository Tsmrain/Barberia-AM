...
export const EditBookingModal: React.FC<EditBookingModalProps> = ({
    isOpen, onClose, booking, onSuccess, barbers, services, branches
}) => {
    ...
    useEffect(() => {
        if (booking && isOpen) {
            ...
            setBranchId(booking.sucursal.id);
            setStatus(booking.estado);
        }
    }, [booking, isOpen]);

    // Update available barbers when branch changes
    useEffect(() => {
        if (booking && isOpen) {
            const filtered = barbers.filter(b =>
                b.sucursalId === branchId || b.id === booking.barbero.id
            );
            setAvailableBarbers(filtered);

            // If current barber is not in new branch, reset barber selection (optional, but good UX)
            // But we keep it if it's the original barber to avoid losing data on initial load
            if (branchId !== booking.sucursal.id && !filtered.find(b => b.id === barberId)) {
                if (filtered.length > 0) setBarberId(filtered[0].id);
            }
        }
    }, [branchId, barbers, booking, isOpen]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Ignore validation if we are just updating status but keeping same time
        const isSameTime = newDate.getTime() === booking.fecha_hora.getTime();

        if (!isSameTime && newDate < new Date()) {
            toast.error("No puedes mover una reserva al pasado", {
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto no-scrollbar">
                    ...
                    <div className="flex items-center space-x-3 bg-white/5 p-3 rounded-xl border border-white/5">
                        ...
                        <div className="flex-1">
                            <p className="text-sm font-bold text-white mb-1.5">{booking.cliente.nombre_completo}</p>
                            <div className="flex items-center">
                                <button
                                    type="button"
                                    onClick={() => {
                                        navigator.clipboard.writeText(booking.cliente.celular);
                                        toast.success("Número copiado al portapapeles");
                                    }}
                                    className="group flex items-center gap-3 bg-[#0a0a0a] hover:bg-white/5 border border-white/10 hover:border-amber-500/50 rounded-lg p-1.5 pr-3 transition-all cursor-pointer shadow-sm active:scale-95"
                                >
                                    <span className="bg-amber-500/10 text-amber-500 px-2 py-1 rounded text-xs font-mono font-bold border border-amber-500/20 group-hover:bg-amber-500/20 transition-colors">
                                        {booking.cliente.celular}
                                    </span>
                                    <div className="flex items-center gap-1.5 opacity-50 group-hover:opacity-100 transition-opacity">
                                        <Copy className="w-3 h-3 text-white" />
                                        <span className="text-[10px] font-medium text-white uppercase tracking-wide">Copiar</span>
                                    </div>
                                </button>
                            </div>
                        </div>
